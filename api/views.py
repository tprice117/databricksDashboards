...
def denver_compliance_report(request):
    ...
    for ticket in order_disposal_tickets:
        order_disposal_tickets_inputs.append({
            ...
            "landfill": ticket.weight if ticket.waste_type.name == "Trash (Household Goods)" else "",
            "wood": ticket.weight if ticket.waste_type.name == "Wood" else "",
            "concrete_brick_block": ticket.weight if ticket.waste_type.name == "Concrete (No metal)" else "",
            "asphalt": ticket.weight if ticket.waste_type.name == "Asphalt" else "",
            "metal": ticket.weight if ticket.waste_type.name == "Mixed Metal" else "",
            "cardboard": ticket.weight if ticket.waste_type.name == "Cardboard" else "",
            "donation_reuse": ticket.weight if ticket.waste_type.name == "Salvage for Donation/Reuse" else "",
            "other": "",
            "total_diversion": "0",
            "total_cd_debris": "0",
