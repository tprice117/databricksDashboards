...
def on_order_update(instance: Order, created: bool, deleted: bool):
    """Sends an event to CustomerIO when an Order is created or updated."""
    ...
    def _send_event():
        try:
            ...
            logger.exception(f"on_order_update: [{instance.id}]-[{event_name}]-[{e}]")
...
def on_order_post_save(sender, instance, created, **kwargs):
    ...
    # This is called from api.models.order.order.py pre_save to ensure line items are saved before this is called.
    on_order_update(instance, created, False)
...
def on_order_pre_delete(sender, instance, using, **kwargs):
    """Sends an event to CustomerIO when an Order is deleted."""

    on_order_update(instance, False, True)
...
@receiver(post_save, sender=UserGroupLegal)
def on_user_group_legal_post_save(sender, instance: UserGroupLegal, created, **kwargs):
    """Sends an event to CustomerIO when a UserGroupLegal is created or updated."""

    if created:
        event_name = "user_group_legal_created"
    else:
        event_name = "user_group_legal_updated"

    def _send_event():
        try:
            # Add attributes to CustomerIO
            event_data = {
                "id": str(instance.id),
                "user_group": str(instance.user_group_id),
                "legal_name": instance.name,
                "legal_address": instance.formatted_address,
                "legal_tax_id": instance.tax_id,
                "legal_accepted_net_terms": instance.accepted_net_terms,
                "legal_years_in_business": instance.years_in_business,
                "legal_doing_business_as": instance.doing_business_as,
                "legal_structure": instance.structure,
                "legal_industry": instance.industry,
            }
            sending_user_email = None
            if instance.updated_by:
                sending_user_email = instance.updated_by.email
            else:
                first_user = instance.user_group.users.first()
                if first_user:
                    sending_user_email = first_user.email

            if sending_user_email:
                update_company(
                    sending_user_email, str(instance.user_group_id), event_data
                )
            else:
                logger.error(
                    f"on_user_group_legal_post_save:{event_name}: [{instance.id}]-[No user to send company to]"
                )
        except Exception as e:
            logger.exception(f"on_user_group_legal_post_save: [{instance.id}]-[{e}]")
...
def on_user_group_credit_application_post_save(
    sender, instance: UserGroupCreditApplication, created, **kwargs
):
    """Sends an event to CustomerIO when a UserGroupCreditApplication is created or updated."""

    if created:
        event_name = "user_group_credit_application_created"
    else:
        event_name = "user_group_credit_application_updated"
    ...
    def _send_event():
        try:
            ...
            event_data = {
                ...
                "accepts_credit_authorization": instance.accepts_credit_authorization,
            }
            sending_user_email = None
            if instance.updated_by:
                sending_user_email = instance.updated_by.email
            ...
            if sending_user_email:
                ...
                logger.error(
                    f"on_user_group_credit_application_post_save:{event_name}: [{instance.id}]-[No user to send company to]"
                )
        except Exception as e:
            logger.exception(
                f"on_user_group_credit_application_post_save: [{instance.id}]-[{e}]"
            )
...
def on_user_post_save(sender, instance: User, created, **kwargs):
    """Sends an event to CustomerIO when a User is created or updated."""
...
def on_user_address_post_save(sender, instance: UserAddress, created, **kwargs):
    """Sends an event to   when a UserAddress is created or updated."""
