...
def index(request):
    ...
    # Translated Measures
    customerAmountCompleted = (Order.objects.filter(status="COMPLETE")
    ...
    #customerAmount
    customerAmount = (OrderLineItem.objects  
    ...
    customerAmount_dict = [float(entry['customerAmnt']) if isinstance(entry['customerAmnt'], Decimal) else entry['customerAmnt'] for entry in customerAmount]
    ...
    customerAmountByDay = (OrderLineItem.objects  
    ...
    customerAmountByDay_dict = [float(entry['customerAmntByDay']) if isinstance(entry['customerAmntByDay'], Decimal) else entry['customerAmntByDay'] for entry in customerAmountByDay]
    ...
    supplierAmountCompleted = (Order.objects.filter(status="COMPLETE")
            .annotate(month=TruncMonth('end_date'))  # Groupby month
            .values('month')
            .annotate(
    ...
    supplierAmount_dict = [float(entry['suppAmount']) if isinstance(entry['suppAmount'], Decimal) else entry['suppAmount'] for entry in supplierAmount]

    #SupplierAmountbyDay
    supplierAmountByDay = (Order.objects
    ...
    supplierAmountByDay_dict = [float(entry['suppAmount']) if isinstance(entry['suppAmount'], Decimal) else entry['suppAmount'] for entry in supplierAmount]
    ...
    customerAmountCompleted_dict = [float(entry['gmv']) if isinstance(entry['gmv'], Decimal) else entry['gmv'] for entry in customerAmountCompleted]
    supplierAmountCompleted_dict = [float(entry['suppAmountCompleted']) if isinstance(entry['suppAmountCompleted'], Decimal) else entry['suppAmountCompleted'] for entry in supplierAmountCompleted]
    
    netRevenueCompletedByMonth = {}
    supp_map = {entry['month']: entry['suppAmountCompleted'] for entry in supplierAmountCompleted}
    ...
    netRevenueCompletedSum = customerAmountCompleted_dictSum - supplierAmountCompleted_dictSum
    context['netRevenueCompleted'] = netRevenueCompletedSum

    #Net Revenue by day
    customerAmountByDay_dict = [float(entry['customerAmntByDay']) if isinstance(entry['customerAmntByDay'], Decimal) else entry['customerAmntByDay'] for entry in customerAmountByDay]
    supplierAmountByDay_dict = [float(entry['suppAmount']) if isinstance(entry['suppAmount'], Decimal) else entry['suppAmount'] for entry in supplierAmountByDay]
    netRevenueByDay = {}
    ...
    # Suppliers Count
    sellerCount = OrderGroup.objects.values('seller_product_seller_location_id').distinct().count()
    ...
    orderSum = Order.objects.annotate(
        caluclated_val=Sum(
            F('order_line_items__rate') * F('order_line_items__quantity') * (1 + F('order_line_items__platform_fee_percent') * 0.01),
            output_field = DecimalField()
        ) 
    )
    ...
    takeRateByMonth = (
        ...
        .annotate(
            ...
            supplierAmnt=Sum(F('rate') * F('quantity'))  # Assuming supplierAmountSum is calculated like this
        )
        .order_by('month')
    )
    takeRateData = []
    for entry in takeRateByMonth:
        ...
        takeRate = round(((customerAmount - supplierAmountSum) / customerAmount) * 100, 1)
    ...
    data = [float(entry['gmv']) if isinstance(entry['gmv'], Decimal) else entry['gmv'] for entry in customerAmountCompleted]
    scheduleddata = [float(entry['gmvScheduled']) if isinstance(entry['gmvScheduled'], Decimal) else entry['gmvScheduled'] for entry in customerAmountScheduled]
    ...
    data3 = [float(entry['customerAmntByDay']) if isinstance(entry['customerAmntByDay'], Decimal) else entry['customerAmntByDay'] for entry in customerAmountByDay]
    ...
    data4 = [float(entry['takeRate']) if isinstance(entry['takeRate'], Decimal) else entry['takeRate'] for entry in takeRateData]
