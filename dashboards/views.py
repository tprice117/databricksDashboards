...
def seller_location_dashboard(request):
        context = {}
        unique_seller_locations_count = SellerLocation.objects.values("id").distinct().count()
        ...
        seller_locations_with_processed_orders = SellerLocation.objects.filter(
        ...
        seller_location_revenue = SellerLocation.objects.annotate(
            revenue=Sum(
                Case(
                    When(
                        seller_product_seller_locations__order_groups__orders__status="COMPLETE",
                        then=F("seller_product_seller_locations__order_groups__orders__order_line_items__rate")
                        * F("seller_product_seller_locations__order_groups__orders__order_line_items__quantity")
                    ),
                    default=Value(0),
                    output_field=DecimalField(),
                )
        ...
        average_revenue = total_revenue / len(seller_location_revenue) if seller_location_revenue else Decimal("0.00")
        ...
        # Prepare data for chart.js
        chart_labels = [entry["month"].strftime("%b-%Y") for entry in seller_locations_by_month]
        ...
        # Prepare data for chart.js
        avg_revenue_chart_labels = [entry["month"].strftime("%b-%Y") for entry in avg_revenue_per_seller_location_by_month]
        avg_revenue_chart_data = [float(entry["avg_revenue"]) for entry in avg_revenue_per_seller_location_by_month]

        context["avg_revenue_per_seller_location_by_month_labels"] = json.dumps(avg_revenue_chart_labels)
        context["avg_revenue_per_seller_location_by_month_data"] = json.dumps(avg_revenue_chart_data)


        context["average_revenue_per_seller_location"] = average_revenue
        context["seller_locations_with_processed_orders"] = seller_locations_with_processed_orders
